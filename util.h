...
struct vector3 * calc_force_grav(struct particle *p1, struct particle *p2) {
  ...
  float numer = G * p1->mass * p2->mass;
  float denom = pow(calc_radius(p1, p2), 2);
...
float calc_radius(struct particle *p1, struct particle *p2) {
  ...
  float dx2 = pow(p1->position->x - p2->position->x, 2);
  float dy2 = pow(p1->position->y - p2->position->y, 2);
  float dz2 = pow(p1->position->z - p2->position->z, 2);
